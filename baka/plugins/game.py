...
# Final Mirror Game Plugin - Fixed Revive | No Start Check | Spy Intelligence
...
async def check_protection_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Detailed Target Intelligence Report (Kill/Rob availability)."""
...
async def revive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Brings user back to life."""
    user = update.effective_user
    user_db = ensure_user_exists(user)

    if user_db.get('status') == 'alive': 
        return await update.message.reply_text(f"✨ {get_mention(user)}, you are already alive!", parse_mode=ParseMode.HTML)

    if user_db.get('balance', 0) < REVIVE_COST: 
        return await update.message.reply_text(f"❌ Revive costs {format_money(REVIVE_COST)}!")

    # Fixed: Full status reset
    ...
    await update.message.reply_text(
        f"❤️ <b>{stylize_text('REVIVED')}!</b>\n✨ {get_mention(user)} is back to life!\n💸 <b>Cost:</b> {format_money(REVIVE_COST)}", 
