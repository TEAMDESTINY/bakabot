...
# Final Game/RPG Plugin - Exact Baka Bot Replica UI (FINAL)
...
# --------------------------------------------------
# Helper: Baka Style Clickable Name (No ID Visible)
# --------------------------------------------------
...
# --------------------------------------------------
# 🔪 KILL COMMAND (Protected-safe + Replica UI)
# --------------------------------------------------
...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not target_db:
        return await update.message.reply_text(
            f"⚠️ {stylize_text('Reply or Tag to kill!')}",
            parse_mode=ParseMode.HTML
        )

    # Self kill block
    if attacker_db["user_id"] == target_db["user_id"]:
        return await update.message.reply_text(
            f"🙃 {stylize_text('Khud pe ye command kaam nahi karti!')}",
            parse_mode=ParseMode.HTML
        )
    ...
    if target_db.get('user_id') == OWNER_ID:
        return await update.message.reply_text(
            f"🙊 <b>{stylize_text('Senpai Shield!')}</b>",
            parse_mode=ParseMode.HTML
        )

    if attacker_db.get('status') == 'dead':
        return await update.message.reply_text(
            f"💀 {stylize_text('Pehle khud revive ho jao!')}",
            parse_mode=ParseMode.HTML
        )

    if target_db.get('status') == 'dead':
        return await update.message.reply_text(
            f"⚰️ {stylize_text('Murdon ko dubara nahi maarte!')}",
            parse_mode=ParseMode.HTML
        )
    ...
    users_collection.update_one(
        {"user_id": attacker_db["user_id"]},
        {"$inc": {"kills": 1, "balance": reward}}
    )
...
# --------------------------------------------------
# 💰 ROB COMMAND (Protected-safe + Exact Output)
# --------------------------------------------------
...
async def rob(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        return await update.message.reply_text(
            f"⚠️ {stylize_text('Usage')}: <code>/rob 100</code>",
            parse_mode=ParseMode.HTML
        )
    ...
    try:
        ...
        return await update.message.reply_text(
            f"⚠️ {stylize_text('Invalid amount!')}",
            parse_mode=ParseMode.HTML
        )
    ...
    target_db, error = await resolve_target(
        update,
        context,
        specific_arg=context.args[1] if len(context.args) > 1 else None
    )

    if not target_db:
        return await update.message.reply_text(
            f"⚠️ {stylize_text('Reply or tag someone to rob!')}",
            parse_mode=ParseMode.HTML
        )
    ...
    if attacker_db["user_id"] == target_db["user_id"]:
        return await update.message.reply_text(
            f"🙃 {stylize_text('Khud ko rob nahi kar sakte!')}",
            parse_mode=ParseMode.HTML
        )
