# FINAL MASTER GAME PLUGIN — 100% WORKING, NO BYPASS
...
# ---------- SAFE EXPIRY ----------
def get_expiry(user):
    expiry = user.get("protection_expiry")
...
async def kill(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if attacker.get("status") == "dead":
        return await update.message.reply_text(
            nezuko_style("you are dead! revive first.")
        )
    ...
    if not victim:
        return await update.message.reply_text(
            nezuko_style("reply to a victim!")
        )

    expiry = get_expiry(victim)
    now = datetime.utcnow()

    # 🔒 HARD BLOCK
    if expiry and expiry > now and update.effective_user.id != OWNER_ID:
        return await update.message.reply_text(
            "🛡️ " + nezuko_style("victim is protected right now!")
        )

    if victim.get("status") == "dead":
        return await update.message.reply_text(
            nezuko_style(f"{victim['name']} is already dead!")
        )
...
async def rob(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message or not context.args:
        return await update.message.reply_text(
            nezuko_style("usage: reply with /rob <amount>")
        )
    ...
    if target.get("balance", 0) < amount:
        return await update.message.reply_text(
            nezuko_style("target doesnt have enough money!")
        )
...
async def revive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target.get("status") == "alive":
        return await update.message.reply_text(
            nezuko_style("user is already alive!")
        )

    if user.get("balance", 0) < REVIVE_COST:
        return await update.message.reply_text(
            f"<b>❌ revive cost: {format_money(REVIVE_COST)}</b>",
            parse_mode=ParseMode.HTML
        )
    ...
    await update.message.reply_text(
        nezuko_style(f"{target['name']} has been revived!")
    )
...
async def protect(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    expiry = get_expiry(user)
    if expiry and expiry > now:
        return await update.message.reply_text(
            "🛡️ " + nezuko_style("you are already protected!")
        )

    if not context.args:
        return await update.message.reply_text(
            nezuko_style("usage: /protect 1d | 2d")
        )
    ...
    await update.message.reply_text(
        "🛡️ " + nezuko_style(f"you are protected for {choice}")
    )
