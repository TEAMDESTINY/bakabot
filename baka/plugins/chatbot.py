...
async def send_ai_sticker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if s and s.stickers: await update.message.reply_sticker(random.choice(s.stickers).file_id)
    except: pass
...
async def call_model_api(provider, messages, max_tokens):
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        try:
            resp = await client.post(conf["url"], json={"model": conf["model"], "messages": messages, "max_tokens": max_tokens}, headers={"Authorization": f"Bearer {conf['key']}"})
...
async def get_ai_response(chat_id, user_input, user_name, model="mistral"):
    ...
    prompt = f"You are BAKA AI. Sweet Hinglish girl. Reply in 1 short sentence only. User: {user_name}"
    
    doc = chatbot_collection.find_one({"chat_id": chat_id}) or {}
    history = doc.get("history", [])
    msgs = [{"role": "system", "content": prompt}] + history[-6:] + [{"role": "user", "content": user_input}]
    
    reply = await call_model_api(active_model, msgs, tokens) or "ᴀᴄʜʜᴀ ᴊɪ? 😊"
    chatbot_collection.update_one({"chat_id": chat_id}, {"$set": {"history": (history + [{"role":"user","content":user_input},{"role":"assistant","content":reply}])[-10:]}}, upsert=True)
    return reply, is_code

async def ai_message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.message
    if not msg: return
    if msg.sticker and (update.effective_chat.type == ChatType.PRIVATE or (msg.reply_to_message and msg.reply_to_message.from_user.id == context.bot.id)):
        return await send_ai_sticker(update, context)
    if not msg.text or msg.text.startswith("/"): return
    
    should = (update.effective_chat.type == ChatType.PRIVATE or (msg.reply_to_message and msg.reply_to_message.from_user.id == context.bot.id) or any(msg.text.lower().startswith(kw) for kw in ["hey", "hi", "baka"]))
    if should:
        await context.bot.send_chat_action(chat_id=update.effective_chat.id, action=ChatAction.TYPING)
        res, code = await get_ai_response(update.effective_chat.id, msg.text, msg.from_user.first_name)
        if code: await msg.reply_text(res, parse_mode=ParseMode.MARKDOWN)
        else:
            await msg.reply_text(nezuko_style(res))
            if random.random() < 0.15: await send_ai_sticker(update, context)

async def ask_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args: return
    res, code = await get_ai_response(update.effective_chat.id, " ".join(context.args), update.effective_user.first_name)
